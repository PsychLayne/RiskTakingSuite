...
class PenguinGame:
    ...
    def update(self):
        ...
        if self.moving and self.state == "play":
            ...
            if self.sled_x >= mid_x and self.success is None:
                if random.randint(1, MAX_FISH) <= self.fish_count:
                    # FAILURE is decided here
                    self.success = False
    ...
    def draw_menu(self):
        ...
        # Instructions
        instructions = [
            ("璽", "Click 'Catch Fish' to add fish to your sled"),
            ("逃", "Each fish increases your potential reward"),
            ("笞�ｸ�", "More fish = higher risk of ice breaking!"),
            ("峭", "Click 'Send Sled' to cross the ice"),
            ("識", f"Complete {MAX_TRIALS} trials to finish"),
            ("庁", "Find the balance between risk and reward!")
        ]
