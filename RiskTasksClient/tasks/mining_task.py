...
TASK_NAME = os.environ.get('TASK_NAME', 'mountain_mining')
...
config, task_config, exp_config = load_task_config('mountain_mining')
...
MAX_ORE = task_config.get('max_ore', 64)
POINTS_PER_ORE = task_config.get('points_per_ore', 5)
...
# Fullscreen setup based on config
fullscreen_mode = config.get('display', {}).get('fullscreen', True)
if fullscreen_mode:
    ...
    NATIVE_SCREEN_WIDTH = GAME_WIDTH
    NATIVE_SCREEN_HEIGHT = GAME_HEIGHT
    screen = pygame.display.set_mode((NATIVE_SCREEN_WIDTH, NATIVE_SCREEN_HEIGHT))
...
class MountainMinerGame:
    def __init__(self):
        ...
        # Pre-determine "snap points" for all trials (selection without replacement)
        self.snap_points = []
    ...
    def generate_snap_points(self):
        """Generate snap points for all trials using selection without replacement."""
        self.snap_points = []
        for _ in range(TOTAL_TRIALS):
            # Each trial has a snap point between 1 and MAX_ORE
            snap_point = random.randint(1, MAX_ORE)
    ...
    def send_bucket(self):
        if self.ore_in_bucket > 0 and not self.bucket_moving:
            ...
            # Selection without replacement: check against predetermined snap point
            snap_threshold = self.snap_points[self.trial - 1]

            # Rope snaps if ore count equals or exceeds the snap point
            if self.ore_in_bucket >= snap_threshold:
                self.rope_snapped = True
                self.snap_point = random.uniform(0.2, 0.5)  # Where along the rope it snaps visually
    ...
    def reset_bucket(self):
        ...
        # Generate new snap points if all trials are complete
        if self.trial > TOTAL_TRIALS:
            self.generate_snap_points()

    def complete_trial(self, success):
        # Log to database
        self.log_trial_to_db(self.ore_in_bucket, success)

        if success:
            self.ore_banked += self.ore_in_bucket
            self.last_result = f"Success! Delivered {self.ore_in_bucket} ore ({self.ore_in_bucket * POINTS_PER_ORE} cents)"
        else:
    ...
    def log_trial_to_db(self, ore_count, success):
        """Log trial data to the database."""
        # Don't log to database in test mode
        if SESSION_ID == 0 or test_mode:
            if test_mode:
                print(f"TEST MODE - Trial {self.trial}: Ore={ore_count}, Success={success}")
            return

        try:
            # Calculate risk level (0-1 scale)
            risk_level = ore_count / MAX_ORE
            ...
            # Points earned
            points = ore_count * POINTS_PER_ORE if success else 0

            # Create additional data - STANDARDIZED FORMAT
            additional_data = {
                # REQUIRED FIELDS FOR ALL TASKS
                "actions": ore_count,  # Number of ore collected (actions taken)
                "action_limit": self.snap_points[self.trial - 1],  # Point where rope would snap
                "potential_points": ore_count * POINTS_PER_ORE,  # Points that could be earned
                "total_banked": self.ore_banked * POINTS_PER_ORE,  # Running total of points banked

                # TASK-SPECIFIC FIELDS
                "ore_mined": ore_count,  # Task-specific alias for actions
                "ore_collected": ore_count,  # Alternative alias for backward compatibility
                "snap_point": self.snap_points[self.trial - 1],  # Task-specific name
                "total_ore_banked": self.ore_banked
            }
    ...
    def save_final_data(self):
        ...
        if SESSION_ID == 0 or test_mode:
            if test_mode:
                print(f"TEST MODE COMPLETE - Total ore banked: {self.ore_banked}")
            return

        try:
            # You could add a summary entry or update session info here
            print(f"Task completed. Total ore banked: {self.ore_banked}")
        except Exception as e:
    ...
    def draw_truck(self, surface):
        ...
        if self.ore_banked > 0:
            total_points = self.ore_banked * POINTS_PER_ORE
            ore_text = large_font.render(f"{total_points}¢", True, WHITE)
    ...
    def draw_menu(self, surface):
        ...
        instructions = [("⛏️", "Click the large rock to mine ore."),
                        ("📦", f"Each ore is worth {POINTS_PER_ORE} cents."),
    ...
    def draw_results(self, surface):
        ...
        surface.blit(score_text, score_text.get_rect(center=(GAME_WIDTH // 2, 280)))

        ore_text = font.render(f"Total Ore Banked: {self.ore_banked}", True, COLOR_UI_TEXT)
        surface.blit(ore_text, ore_text.get_rect(center=(GAME_WIDTH // 2, 340)))
        ...
        surface.blit(info_text, info_text.get_rect(center=(GAME_WIDTH // 2, 400)))

        # Exit instruction
        exit_text = font.render("Press ESC to exit.", True, COLOR_UI_TEXT)
        surface.blit(exit_text, exit_text.get_rect(center=(GAME_WIDTH // 2, GAME_HEIGHT - 200)))
