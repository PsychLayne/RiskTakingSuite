...
if config_path.exists():
    with open(config_path, 'r') as f:
        config = json.load(f)
        task_config = config.get('tasks', {}).get('spinning_bottle', {})
else:
...
class RiskWheel:
    ...
    def log_trial_to_db(self, risk_level, points_earned, outcome, reaction_time):
        ...
        try:
            ...
            additional_data = {
                "red_segments": len(self.red_segments),
                "total_segments": self.segments,
                "landing_segment": int(self.bottle_angle % 360 // self.segment_angle),
                "potential_points": self.potential_points_current_trial,
                "total_score_so_far": self.points,
                "win_color": WIN_COLOR,
