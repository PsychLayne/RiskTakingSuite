...
Enhanced to support single task sessions and repeated tasks with different parameters.
"""
...
class ExperimentBuilder(ctk.CTkFrame):
    ...
    def create_parameters_section_redesigned(self, parent):
        """Create experiment parameters section with cleaner layout."""
        # Section header
        header_frame = ctk.CTkFrame(parent, fg_color="transparent")
        ...
        ctk.CTkEntry(trials_card, textvariable=self.trials_var, width=100).pack(pady=(0, 10))
        ...
        ctk.CTkEntry(gap_card, textvariable=self.gap_var, width=100).pack(pady=(0, 10))

        # Tasks per session card - NOW SUPPORTS 1 TASK
    ...
    def build_experiment_config(self) -> Dict:
        ...
        config = {
            "experiment": {
                ...
                "tasks_per_session": self.tasks_per_session_var.get(),
                "enabled_task_instances": enabled_tasks,  # Now using instance IDs
                "task_sequence": {
            ...
            "task_instances": task_configs  # Store instance configs
        }
