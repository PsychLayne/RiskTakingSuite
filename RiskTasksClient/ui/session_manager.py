...
class SessionManager(ctk.CTkFrame):
        ...
        if can_schedule:
            ...
            try:
                ...
                if not assigned_tasks:
                    # Assign tasks now if not already assigned
                    assigned_tasks = self.task_scheduler.assign_tasks_for_participant(
                        self.current_participant_id,
                        next_session
                    )
                ...
                info_text = f"❌ Cannot schedule session: {e}"
                self.start_session_button.configure(state="disabled")
        else:
    ...
    def launch_task(self, task_name: str, session_id: int):
        ...
        task_files = {
            "bart": "BARTClassic.py",
            "ice_fishing": "ice3.py",
            "mountain_mining": "MiningRiskTaskCLICK.py",
            "spinning_bottle": "STB_Button.py"
        }
